...
public class CommandListener extends ListenerAdapter {
    ...
    private void disconnect(SlashCommandInteractionEvent event){
        if(author.getVoiceState().inAudioChannel() && bots.containsKey(guild)){
    ...
    private void rolepicker(SlashCommandInteractionEvent event){
    ...
    private void version(SlashCommandInteractionEvent event){
        String msg = "**Changelog**\n```\ngeneral cleanup and restructuring of code\n```";
        event.reply(msg).setEphemeral(true).queue();
    }

    private void team(SlashCommandInteractionEvent event){
        if(author.hasPermission(Permission.ADMINISTRATOR)){
    ...
    private void spawnmessage(SlashCommandInteractionEvent event){
        if(author.hasPermission(Permission.ADMINISTRATOR)){
    ...
    private void requirements(SlashCommandInteractionEvent event){
        if(author.hasPermission(Permission.ADMINISTRATOR)){
            EmbedBuilder builder = new EmbedBuilder();
            switch(event.getSubcommandName()){
                case "add":
                ...
                case "form":
                try {
                    JSONObject raidForm = Config.getFile("team_requirements.json").getJSONObject("raid_form");
                    String filled = "";
    ...
    private void featurerequest(SlashCommandInteractionEvent event){
